...
- Quiet checks keep running while an update is on offer and after a failed update, so a newer version shows up without restarting PeekOCR. The card only changes when a newer version is actually found, a failed update keeps its Retry, and nothing is ever installed without "Install now".
